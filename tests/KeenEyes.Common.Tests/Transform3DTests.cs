...
/// <summary>
/// Tests for the Transform3D component and its extensions.
/// </summary>
public class Transform3DTests
{
    ...
    [Fact]
    public void Matrix_Identity_ReturnsIdentityMatrix()
    {
        var transform = Transform3D.Identity;
    ...
    [Fact]
    public void Matrix_WithPosition_HasCorrectTranslation()
    {
        var position = new Vector3(10, 20, 30);
        var transform = new Transform3D(position, Quaternion.Identity, Vector3.One);
    ...
    [Fact]
    public void Matrix_WithScale_HasCorrectScaling()
    {
        var scale = new Vector3(2, 3, 4);
        var transform = new Transform3D(Vector3.Zero, Quaternion.Identity, scale);
    ...
    [Fact]
    public void Matrix_TransformsPointCorrectly()
    {
    ...
    [Fact]
    public void Matrix_WithRotation_TransformsPointCorrectly()
    {
        ...
        // Point at (1,0,0) rotated 90 degrees around Y = (0,0,-1)
        Assert.True(MathF.Abs(transformed.X) < Epsilon);
        Assert.Equal(0f, transformed.Y, Epsilon);
        Assert.Equal(-1f, transformed.Z, Epsilon);
    }

    [Fact]
    public void Matrix_WithComplexTransform_CombinesCorrectly()
    {
        ...
        var transform = new Transform3D(position, rotation, scale);

        var matrix = transform.Matrix();
        var point = new Vector3(1, 0, 0);
        var transformed = Vector3.Transform(point, matrix);

        // Verify the transformation applied (scale, then rotate, then translate)
        var expected = Vector3.Transform(point * scale, rotation) + position;

        Assert.Equal(expected.X, transformed.X, Epsilon);
        Assert.Equal(expected.Y, transformed.Y, Epsilon);
        Assert.Equal(expected.Z, transformed.Z, Epsilon);
    }
    ...
    public void Forward_Rotated90DegreesAroundY_IsNegativeX()
    {
        ...
        Assert.True(MathF.Abs(forward.Y) < Epsilon);
        Assert.True(MathF.Abs(forward.Z) < Epsilon);
    }

    [Fact]
    public void Right_Rotated90DegreesAroundY_IsNegativeZ()
    {
        var rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI / 2f);
        var transform = new Transform3D(Vector3.Zero, rotation, Vector3.One);
        ...
        Assert.True(MathF.Abs(right.X) < Epsilon);
        Assert.Equal(0f, right.Y, Epsilon);
        Assert.Equal(-1f, right.Z, Epsilon);
    }

    [Fact]
    public void Up_Rotated90DegreesAroundX_IsNegativeZ()
    {
        ...
        Assert.Equal(0f, up.X, Epsilon);
        Assert.True(MathF.Abs(up.Y) < Epsilon);
        Assert.Equal(1f, up.Z, Epsilon);
    }

    [Fact]
    public void DirectionVectors_AreOrthogonal()
    {
        var rotation = Quaternion.CreateFromYawPitchRoll(1.2f, 0.5f, 0.8f);
        var transform = new Transform3D(Vector3.Zero, rotation, Vector3.One);
        ...
        // All pairs should be orthogonal (dot product near zero)
        Assert.True(MathF.Abs(Vector3.Dot(forward, right)) < Epsilon);
        Assert.True(MathF.Abs(Vector3.Dot(forward, up)) < Epsilon);
        Assert.True(MathF.Abs(Vector3.Dot(right, up)) < Epsilon);
    }
    ...
    [Fact]
    public void DirectionVectors_RightCrossForward_GivesUp()
    {
        var rotation = Quaternion.CreateFromYawPitchRoll(1.2f, 0.5f, 0.8f);
        var transform = new Transform3D(Vector3.Zero, rotation, Vector3.One);
        ...
        // Verify the cross product relationship: right x forward = up
        var cross = Vector3.Cross(right, forward);

        Assert.Equal(up.X, cross.X, Epsilon);
        Assert.Equal(up.Y, cross.Y, Epsilon);
        Assert.Equal(up.Z, cross.Z, Epsilon);
    }
    ...
    {
        var transform = Transform3D.Identity;
        var newPosition = new Vector3(5, 10, 15);
    ...
    [Fact]
    public void Rotation_CanBeModified()
    {
        var transform = Transform3D.Identity;
        var newRotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI / 3f);
