...
public partial class SpawnerSystem : SystemBase
{
    ...
    private void SpawnEnemy()
    {
        // Random edge spawn
        float x, y, vx, vy;
        int edge = World.NextInt(EdgeCount);
        ...
        switch (edge)
        {
            case 0: // Top
                ...
                y = 0;
                vx = (World.NextFloat() - 0.5f) * BaseVelocityMultiplier;
                vy = World.NextFloat() * DirectionalVelocityMultiplier + MinimumInwardVelocity;
                break;
            case 1: // Bottom
                x = World.NextFloat() * WorldWidth;
                y = WorldHeight - 1;
            ...
            case 2: // Left
                ...
                vx = World.NextFloat() * DirectionalVelocityMultiplier + MinimumInwardVelocity;
                vy = (World.NextFloat() - 0.5f) * BaseVelocityMultiplier;
                break;
            default: // Right
                x = WorldWidth - 1;
                y = World.NextFloat() * WorldHeight;
...
public partial class EnemyShootingSystem : SystemBase
{
    ...
    public override void Update(float deltaTime)
    {
        ...
        foreach (var entity in World.Query<Position, Cooldown>().With<Enemy>().Without<Dead>())
        {
            ...
            if (cooldown.Remaining <= 0)
            {
                cooldown.Remaining = BaseEnemyFireRate + World.NextFloat();
